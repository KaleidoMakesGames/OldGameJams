...
namespace KMGPathfinding {
    public class Graph<Node> where Node : class {
        private Dictionary<Node, List<Node>> _neighbors;

        public delegate float BinaryFunc(Node a, Node b);
        public delegate bool CanOccupyFunc(Node a);
        ...
        public List<Node> AStarPath(Node start, Node goal, BinaryFunc costFunc, BinaryFunc heuristicFunc, CanOccupyFunc canOccupyFunc) {
            ...
            Dictionary<Node, bool> canOccupyCached = new Dictionary<Node, bool>();
            ...
            while (openSet.Count > 0) {
                ...
                foreach(var neighbor in _neighbors[current]) {
                    if(canOccupyCached.ContainsKey(neighbor)) {
                        if(!canOccupyCached[neighbor]) {
                            continue;
                        }
                    } else {
                        canOccupyCached[neighbor] = canOccupyFunc(neighbor);
                    }
